...
| `discovery.redpanda` | `internal/component/discovery/redpanda/` | Enriches discovered pod targets with per-pod Redpanda cluster UUIDs. Broker-to-collector ownership is decided one of two ways, selected by the `clustering` argument: `false` (the default) is a static `ordinal % NumShards` scheme with no dependency on Alloy's own clustering; `true` dynamically allocates brokers via an embedded Raft group (one peer per replica, membership driven by Alloy's native `cluster.Peers()`), with a `preStop`-hook-driven graceful leave on scale-down. Switching an existing deployment between modes needs a coordinated, all-at-once rollout of every replica (see the package doc comment for why). Raft mode requires a dedicated `raft_bind_port` per replica, separate from Alloy's own clustering port. Bare-metal/non-Kubernetes discovery sources aren't supported yet in Raft mode. Stability: `generally-available`. |
